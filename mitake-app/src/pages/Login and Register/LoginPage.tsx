...
import {
  ...
  alertaRecuperacionForm,
  alertaRecuperacionEnviada,
...
function LoginPage({ alIniciarSesion, alIrARegistro }: PropiedadesDeLoginPage) {
  ...
  async function manejarEnvio(e: React.FormEvent) {
    ...
    setCargando(true);
     try {
      const usuarioAutenticado = await iniciarSesionConEmail(email, password);
      await alertaLoginExitoso(usuarioAutenticado.displayName ?? email);
      alIniciarSesion(email);
    } catch (errorDeFirebase: unknown) {
      const codigo = (errorDeFirebase as { code?: string }).code ?? "";
      await alertaErrorDeAutenticacion(obtenerMensajeDeError(codigo));
    } finally {
      setCargando(false);
    }
  }
  ...
  // ---- Login con Google ----
  async function manejarLoginConGoogle() {
    setCargandoGoogle(true);
    try {
      const usuarioDeGoogle = await iniciarSesionConGoogle();
      await alertaLoginExitoso(usuarioDeGoogle.displayName ?? usuarioDeGoogle.email ?? "Usuario");
      alIniciarSesion(usuarioDeGoogle.email ?? "");
    } catch (errorDeFirebase: unknown) {
      const codigo = (errorDeFirebase as { code?: string }).code ?? "";
      // Si cerró el popup no mostramos error
  ...
  async function manejarRecuperarContrasenia() {
    const emailIngresado = await alertaRecuperacionForm();
    if (!emailIngresado) return;

    try {
      await enviarEmailDeRecuperacion(emailIngresado);
      await alertaRecuperacionEnviada(emailIngresado);
    } catch (errorDeFirebase: unknown) {
      const codigo = (errorDeFirebase as { code?: string }).code ?? "";
      await alertaErrorDeAutenticacion(obtenerMensajeDeError(codigo));
    }
    ...
    <div className="auth-layout">
       ...
       <div className="auth-layout__form-wrap">
        <form className="auth-form" onSubmit={manejarEnvio} noValidate>
          ...
          <div className="auth-form__encabezado">
           ...
           <div className="auth-campo">
            <div className="auth-campo__label-fila">
              ...
              {/* ── CONECTADO: abre popup SweetAlert2 con input de email ── */}
              <button
                type="button"
                className="auth-campo__link"
                onClick={manejarRecuperarContrasenia}
              >
          ...
          {/* ── CONECTADO: llama a iniciarSesionConGoogle() ── */}
          <button
            type="button"
